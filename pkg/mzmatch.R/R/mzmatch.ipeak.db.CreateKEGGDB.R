mzmatch.ipeak.db.CreateKEGGDB <- function(JHeapSize=1425,i=NULL, inchi=NULL, o=NULL, h=NULL, v=NULL)
{
	...
	## setup the tool
	tool <- paste(mzmatch, "mzmatch.ipeak.db.CreateKEGGDB")
		if (!is.null(i))
		tool <- paste(tool, "-i", i)
	if (!is.null(inchi))
		tool <- paste(tool, "-inchi", inchi)
	if (!is.null(o))
		tool <- paste(tool, "-o", o)
	if (!is.null(h) && h==T)
		tool <- paste(tool, "-h")
