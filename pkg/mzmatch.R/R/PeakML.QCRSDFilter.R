PeakML.QCRSDFilter <- function (filename, ionisation="detect", Rawpath=NULL, outputfile, QCSample=NULL, RSD=0.3, writeRejected=FALSE, sampleFraction=0.7)
{
	...
	to.remove <- append (to.remove, which(QC.RSD>=RSD | na.count >= sampleFraction))
