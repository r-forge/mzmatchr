mzmatch.ipeak.convert.ThermoToML <- function(JHeapSize=1425,i=NULL, o=NULL, format=NULL, centroid=NULL, h=NULL, v=NULL)
{
	...
	## setup the tool
	tool <- paste(mzmatch, "mzmatch.ipeak.convert.ThermoToML")
		if (!is.null(i))
		tool <- paste(tool, "-i", i)
	if (!is.null(o))
		tool <- paste(tool, "-o", o)
	if (!is.null(format))
		tool <- paste(tool, "-format", format)
	if (!is.null(centroid) && centroid==T)
		tool <- paste(tool, "-centroid")
	if (!is.null(h) && h==T)
		tool <- paste(tool, "-h")
	if (!is.null(v) && v==T)
		tool <- paste(tool, "-v")
