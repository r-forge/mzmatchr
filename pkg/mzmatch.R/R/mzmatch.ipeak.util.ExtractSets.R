mzmatch.ipeak.util.ExtractSets <- function(JHeapSize=1425,i=NULL, o=NULL, setnames=NULL, h=NULL, v=NULL)
{
	...
	## setup the tool
	tool <- paste(mzmatch, "mzmatch.ipeak.util.ExtractSets")
		if (!is.null(i))
		tool <- paste(tool, "-i", i)
	if (!is.null(o))
		tool <- paste(tool, "-o", o)
	if (!is.null(setnames))
		tool <- paste(tool, "-setnames", setnames)
	if (!is.null(h) && h==T)
		tool <- paste(tool, "-h")
	if (!is.null(v) && v==T)
		tool <- paste(tool, "-v")
