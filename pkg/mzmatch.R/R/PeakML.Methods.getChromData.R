...
{	
	...
	#intensities <- sapply(getNodeSet(PeakMLtree,"/peakml/peaks/peak/peaks/peak/peakdata/intensities"),xmlValue)
	#masses <- sapply(getNodeSet(PeakMLtree,"/peakml/peaks/peak/peaks/peak/peakdata/masses"),xmlValue)
	#scanids <- sapply(getNodeSet(PeakMLtree,"/peakml/peaks/peak/peaks/peak/peakdata/scanids"),xmlValue)
	#measurementids <- sapply(getNodeSet(PeakMLtree,"/peakml/peaks/peak/peaks/peak/peakdata/measurementids"),xmlValue)
	#})
	#system.time(retentiontimes <- lapply (1:length(retentiontimes),decode64,object=retentiontimes,what="double",size=4,endian="little"))

	#system.time(chromDataList <- lapply (1:length(masses),chromBuild))
