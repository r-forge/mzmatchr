mzmatch.ipeak.util.ReadWSplit <- function(JHeapSize=1425,i=NULL, o=NULL, fullms=NULL, h=NULL, v=NULL)
{
	...
	## setup the tool
	tool <- paste(mzmatch, "mzmatch.ipeak.util.ReadWSplit")
		if (!is.null(i))
		tool <- paste(tool, "-i", i)
	if (!is.null(o))
		tool <- paste(tool, "-o", o)
	if (!is.null(fullms) && fullms==T)
		tool <- paste(tool, "-fullms")
	if (!is.null(h) && h==T)
		tool <- paste(tool, "-h")
	if (!is.null(v) && v==T)
		tool <- paste(tool, "-v")
